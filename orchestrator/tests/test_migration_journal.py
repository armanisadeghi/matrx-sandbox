...
async def test_journal_claim_is_exclusive_and_fences_lifecycle_and_stale_save():
    ...
    assert not await store.claim_terminal_cleanup(
        source.sandbox_id, container_id="source-id", status="stopped", deleted=False
    )
    assert (await store.get(source.sandbox_id)).status == SandboxStatus.RUNNING
    claims = await asyncio.gather(
        *[
            store.claim_migration(
                source.sandbox_id,
                op_id=op,
    ...
    with pytest.raises(RuntimeError, match="migration"):
        await store.save(source)
    assert not await store.advance_migration(
        source.sandbox_id, op_id="wrong", expected_phase="claimed", phase="verified", patch={}
    )
    assert await store.advance_migration(
        source.sandbox_id,
        op_id=journal["op_id"],
        expected_phase="claimed",
        phase="verified",
        patch={"candidate_id": "candidate-id"},
    )
    assert await store.commit_migration(
        source.sandbox_id,
        op_id=journal["op_id"],
