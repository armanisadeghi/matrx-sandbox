...
async def _migrate_hosted_ordered(
    sandbox_id: str, *, old, target: str, env: list[str], volumes: dict,
    labels: dict, host: dict, cur, store, verify_timeout: int,
    platform_env_changes: int,
) -> dict:
    """Fail-closed volume migration: quiesce, snapshot, boot, CAS, then reap.

    The host journal outlives this process.  It is the authority during deploy
    crashes; process-local activity only gates ordinary in-flight requests.
    """
    from orchestrator import activity
    from orchestrator.hosted_migration import (
        HostedMigrationJournal, HostedMigrationStateError, restore_volume, snapshot_volume,
    )
    from orchestrator.sandbox_manager import _get_docker_client
    ...
    try:
        ...
        if journal.inflight(sandbox_id):
            return {"status": "recovery_required", "sandbox_id": sandbox_id,
                    "reason": "hosted migration journal is non-terminal; operator recovery required"}
    except HostedMigrationStateError as exc:
        return {"status": "failed", "sandbox_id": sandbox_id, "reason": str(exc)}

    volume = next((name for name, spec in volumes.items() if spec.get("bind") == "/home/agent"), None)
    if not volume:
        return {"status": "unsupported_storage", "sandbox_id": sandbox_id,
                "reason": "hosted migration requires a named /home/agent volume"}
    client = _get_docker_client()
    backup_volume = f"matrx-migration-backup-{sandbox_id}-{int(time.time())}"
    tmp_name = f"{sandbox_id}-mig"
    old_name = old.name
    old_id = old.id
    record = {"sandbox_id": sandbox_id, "phase": "admitted", "old_id": old_id,
              "old_name": old_name, "target_name": tmp_name, "volume": volume,
              "backup_volume": backup_volume, "target_image": target}
    new = None
    old_renamed = f"{sandbox_id}-old-{int(time.time())}"
    await activity.mark_migrating(sandbox_id)
    try:
        # The durable home is per-user, not per-sandbox. Refuse a migration
        # when any sibling still writes it; stopping that sibling would be an
        # unauthorized lifecycle action and restoring a snapshot over it loses
        # data. The second flock serializes two distinct sandbox IDs sharing a
        # volume across process restarts.
        if await _hosted_volume_has_other_writer(client, volume, old.id):
            return {"status": "busy_deferred", "sandbox_id": sandbox_id,
                    "reason": "shared home volume has another active container"}
        with journal.lock(sandbox_id), journal.lock(f"volume-{volume}"):
            if await _hosted_volume_has_other_writer(client, volume, old.id):
                return {"status": "busy_deferred", "sandbox_id": sandbox_id,
                        "reason": "shared home volume gained another active container"}
            journal.write(record)
            if not await activity.drain_inflight(sandbox_id, timeout=20.0):
                record["phase"] = "recovered"; journal.write(record)
                return {"status": "busy_deferred", "sandbox_id": sandbox_id,
                        "reason": "in-flight calls did not drain"}
            try:
                await asyncio.to_thread(old.stop, timeout=await knob_int("shutdown_timeout_seconds"))
                await asyncio.to_thread(old.reload)
                if getattr(old, "status", None) not in {"exited", "created"}:
                    raise RuntimeError(f"old container did not stop (status={old.status})")
            except Exception as exc:
                record.update(phase="failed_stop", error=str(exc)); journal.write(record)
                return {"status": "failed", "sandbox_id": sandbox_id,
                        "reason": f"old container stop was not verified: {exc}"}
            record["phase"] = "old_stopped"; journal.write(record)
            try:
                await asyncio.to_thread(client.volumes.create, backup_volume, labels={"matrx.migration": sandbox_id})
                image = (old.attrs.get("Config") or {}).get("Image") or target
                await snapshot_volume(client, volume=volume, backup_volume=backup_volume, image=image)
            except Exception as exc:
                record.update(phase="failed_backup", error=str(exc)); journal.write(record)
                return {"status": "failed", "sandbox_id": sandbox_id,
                        "reason": f"durable home backup failed; old retained stopped: {exc}"}
            record["phase"] = "backup_verified"; journal.write(record)
            run_kwargs = dict(image=target, name=tmp_name, detach=True, environment=env,
                              volumes=volumes, network=settings.docker_network, ports={"22/tcp": None},
                              extra_hosts={"host.docker.internal": "host-gateway"}, labels=labels,
                              restart_policy={"Name": "no", "MaximumRetryCount": 0})
            run_kwargs.update(container_runtime_isolation(labels.get("matrx.template"), labels.get("matrx.tier")))
            if host.get("NanoCpus"): run_kwargs["nano_cpus"] = host["NanoCpus"]
            if host.get("Memory"): run_kwargs["mem_limit"] = host["Memory"]
            try:
                new = await asyncio.to_thread(lambda: client.containers.run(**run_kwargs))
                record.update(phase="target_booted", target_id=new.id); journal.write(record)
                ready = await _wait_container_ready(new, verify_timeout, labels.get("matrx.template"))
                new_ver = await _container_version(new) if ready else None
                if not ready or (cur.version is not None and new_ver != cur.version):
                    raise RuntimeError("target readiness/version verification failed")
            except Exception as exc:
                return await _hosted_rollback(journal, record, old, new, client, volume, backup_volume, target, exc)
            try:
                await asyncio.to_thread(old.rename, old_renamed)
                await asyncio.to_thread(new.rename, sandbox_id)
                record.update(phase="names_cut_over", old_name=old_renamed); journal.write(record)
                committed = await store.replace_container_if_current(sandbox_id, old_id, new.id, cur.version or new_ver)
                readback = await store.get(sandbox_id)
                if not committed or not readback or readback.container_id != new.id:
                    raise RuntimeError("durable routing CAS/readback failed")
            except Exception as exc:
                return await _hosted_rollback(journal, record, old, new, client, volume, backup_volume, target, exc)
            record["phase"] = "committed"; journal.write(record)
            try:
                await asyncio.to_thread(old.remove, force=True)
            except APIError as exc:
                logger.error("hosted migrate %s committed but old cleanup failed: %s", sandbox_id, exc)
            return {"status": "migrated", "sandbox_id": sandbox_id, "to_version": cur.version or new_ver,
                    "to_image": target, "platform_env_changed": platform_env_changes}
    except HostedMigrationStateError as exc:
        return {"status": "failed", "sandbox_id": sandbox_id, "reason": str(exc)}
    finally:
        await activity.release_migration(sandbox_id)
...
async def _hosted_rollback(journal, record, old, new, client, volume, backup_volume, image, failure: Exception) -> dict:
    """Restore durable home before restarting old; preserve artifacts on failure."""
    from orchestrator.hosted_migration import restore_volume
    try:
        if new is not None:
            await asyncio.to_thread(new.remove, force=True)
        if await _hosted_volume_has_other_writer(client, volume, old.id):
            raise RuntimeError("refusing restore: shared home gained another active writer")
        await restore_volume(client, volume=volume, backup_volume=backup_volume, image=image)
        if old.name != record.get("sandbox_id"):
            await asyncio.to_thread(old.rename, record["sandbox_id"])
        await asyncio.to_thread(old.start)
        record.update(phase="recovered", error=str(failure)); journal.write(record)
        return {"status": "failed", "sandbox_id": record["sandbox_id"], "reason": f"migration rolled back: {failure}"}
    except Exception as recovery_exc:
        record.update(phase="failed_recovery", error=f"{failure}; rollback: {recovery_exc}")
        journal.write(record)
        return {"status": "recovery_required", "sandbox_id": record["sandbox_id"],
                "reason": "migration failed and rollback could not be completed; retained artifacts require recovery"}


async def recover_hosted_migrations(*, store) -> dict:
    """Resolve journaled hosted migrations before normal reconciliation runs."""
    from orchestrator import activity
    ...
    try:
        ...
        logger.error("Hosted migration recovery unavailable: %s", exc)
        return {"recovered": [], "failed": [str(exc)]}
    client = _get_docker_client()
    recovered: list[str] = []
    failed: list[str] = []
    for record in records:
        ...
        try:
            ...
            # A crash after CAS is committed state even if cleanup never ran.
            if target_id and current and current.container_id == target_id:
                record["phase"] = "committed"; journal.write(record); recovered.append(sandbox_id); continue
            old = await asyncio.to_thread(client.containers.get, record["old_id"])
            try:
                new = await asyncio.to_thread(client.containers.get, target_id) if target_id else None
            except NotFound:
                new = None
            result = await _hosted_rollback(
                journal, record, old, new, client, record["volume"], record["backup_volume"],
                record["target_image"], RuntimeError("orchestrator restarted during hosted migration"),
            )
            (recovered if result["status"] == "failed" else failed).append(sandbox_id)
        except Exception as exc:
            logger.exception("Hosted migration recovery failed for %s", sandbox_id)
...
async def _hosted_volume_has_other_writer(client, volume: str, old_id: str) -> bool:
    """True only for a *running* non-old container mounting this exact volume."""
    try:
        ...
        for candidate in containers:
            ...
            if getattr(candidate, "status", None) not in {"running", "created", "restarting"}:
                continue
            mounts = (candidate.attrs or {}).get("Mounts") or []
            if any(mount.get("Name") == volume for mount in mounts):
                return True
        return False
    except Exception as exc:
        # A partial Docker census cannot prove exclusive write access.
        logger.error("hosted migration volume census failed for %s: %s", volume, exc)
        return True
