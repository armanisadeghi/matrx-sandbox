...
### Basic CRUD
- `GET /fs/list?path=/home/agent&recursive=false&depth=1&pattern=*.py&limit=1000&pageToken=...` — bounded directory listing. `depth` counts immediate children as level 1 (max 32); symlinked directories are never traversed. `pattern` is a case-sensitive glob matched against both the basename and `/`-normalized relative path. `limit` defaults to 1000 and is capped at 5000. Returns `{ entries: [{ name, path, kind: "file"|"dir"|"symlink", size, mtime, mode, target? }, ...], truncated, nextPageToken, scanned }`. Results are binary-lexically ordered. Each single-use `nextPageToken` continues a server-side snapshot for the same query and expires after five minutes. On an exceptionally large directory or sparse pattern, a bounded scan can return an empty `entries` progress page with `truncated=true`; continue with its token.
- `GET /fs/stat?path=...`
