...
| **Hosted tier** (`tier: "hosted"`) | This server `/srv` | Orchestrator at `https://orchestrator.dev.codematrx.com` (deployed via [/srv/apps/sandbox-orchestrator/](/srv/apps/sandbox-orchestrator/)) + dynamically-spawned sandbox containers. **Per-user Docker volumes** (`matrx-user-<uid>` mounted at `/home/agent`) survive container destroy. In-memory store today; Postgres-backed is a follow-up. | `cd /srv/apps/sandbox-orchestrator && docker compose up -d` |
