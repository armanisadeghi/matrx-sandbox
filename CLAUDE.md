...
| **Persistent Cloud Browser plan** (personal/org shared profiles, locked sandbox sidecars, minimal streamed UI, WebRTC takeover, measured capacity) | **[/Users/armanisadeghi/code/common-docs/projects/persistent-cloud-browser/PLAN.md](/Users/armanisadeghi/code/common-docs/projects/persistent-cloud-browser/PLAN.md)** |
...
- **Package / Implementation Separation — read before touching ANY database or connection config.** The package is CAPABLE, the implementation CHOOSES: a `matrx-*` package must be able to run on its own database, and our implementation points every one of them at the SAME Postgres (Matrx Main). **Banned: a second candidate for a connection** — one required set of variables (`SUPABASE_MATRIX_HOST/_PORT/_DATABASE_NAME/_USER/_PASSWORD`, `_SSL` for TLS), raise if absent; pointing something at a different database is a change of VALUES, never a new variable name. ⚠️ This repo's orchestrator predates that rule and reaches the SAME platform Supabase through its own `MATRX_DATABASE_URL` + `MATRX_SANDBOX_STORE` toggle ([orchestrator/orchestrator/config.py:59](orchestrator/orchestrator/config.py), [store.py:756](orchestrator/orchestrator/store.py)) — a known second name for the one connection; don't copy the pattern, and don't add a third. Connection strings this repo handles for OTHER services (Coolify/host-managed Postgres, the docker-best-practices templates) are not covered by the rule. System of record: `/Users/armanisadeghi/code/common-docs/policies/package-vs-implementation.md`.
- **🚨 The orchestrator REFUSES TO START on a bad store config — `MATRX_SANDBOX_STORE` has NO default.** Unset, misspelled, or `memory` on a deployed host ⇒ `RuntimeError` at boot naming the variable, the accepted values (`postgres` | `memory`) and the fix ([config.py `resolve_sandbox_store`](orchestrator/orchestrator/config.py), enforced in [store.py `create_store`](orchestrator/orchestrator/store.py), resolved first thing in `main.py`'s lifespan so no `except` swallows it). It used to default to `memory`: a host that never set the var booted happily on an in-memory store, lost every `sandbox_instances` row on restart, and said so in a `logger.info`. **`memory` is a local-dev/test opt-in only** — it requires `MATRX_STAGE=local` (or a pytest run), and when chosen it prints a boot BANNER and reports `store_backend`/`durable_storage` on `/health`. `is_deployed_host` fails CLOSED: an unset/unknown `MATRX_STAGE` counts as deployed. Both deploy scripts pre-flight `MATRX_SANDBOX_STORE=postgres` so the refusal lands before the container swap, not after. Guard tests: [tests/test_store_config_guard.py](orchestrator/tests/test_store_config_guard.py).
- **No silent defaults, generally.** Any other config value whose *absence* degrades a deployed host (S3 bucket, hosted-tier AWS creds, AI Dream service token, access-token secret, host tier) is listed in `main.py::_degraded_config_warnings` and screams `DEGRADED CONFIG:` at boot. Those stay non-fatal — none destroys data that already exists — but none is silent. Add a new optional-but-consequential setting → add it there.
- **API key auth is enforced by middleware** — `MATRX_API_KEY` unset = unauthenticated mode (a screaming boot banner on a deployed host, a plain warning locally). Don't run that in production.
