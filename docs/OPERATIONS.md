...
🚨 **Isolation (incident 2026-09-13 + XT-10 2026-09-18, [incidents/2026-09-13-platform-env-leak.md](incidents/2026-09-13-platform-env-leak.md)):** the passthrough reaches ONLY the `aidream` template — every other template gets nothing from the orchestrator's environment — and even for `aidream` the forward set is a FAIL-CLOSED ALLOWLIST (`sandbox_manager.PLATFORM_ENV_ALLOWLIST`: the public JWKS document URL, `MATRX_ENV`/`LOG_LEVEL`/`DEBUG`, AWS regions, PATH-style basics). Every other registry name present on the host is dropped and NAMED in the boot report — read them in the orchestrator log line "platform env withheld", or on `GET /aidream-passthrough` (`withheld_with_knob_off`). It used to be a pattern denylist, which is fail-open by construction: it missed 14 secret-shaped names (`ANTHROPIC_KEY`, `MATRX_AGENT_TOKEN`, `MATRX_SCRAPER_TOKEN`, five `SUPABASE_*_KEY`, …) and reported `denied_count=72` while a live `sk-ant-api03-…` sat in a user's container. NEVER fix a missing name by adding a pattern. The knob `infrastructure.sandbox.aidream_template_forwards_master_credentials` (default OFF; a missing row is OFF) is the ONE explicit widening: it hands the whole registry to the box for a short supervised session, and the boot log says so. `GET /sandboxes/{id}/diagnostics` → `platform_env_leaked_count` shows a box still carrying names it should not have (born before a fix): migrate or recreate it. **Env VALUES are never returned by any orchestrator route** — `agent-env` answers with names, presence and length.
